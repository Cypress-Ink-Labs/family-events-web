...
//   project    : Linear project name (default "Family Events")
...
cd ${REPO}. Run \`gh pr checks ${PR} --watch --interval 20\` (it blocks until all checks finish; it may exit non-zero if any check fails — that's fine, inspect after). Then read the final states with \`gh pr checks ${PR}\`.

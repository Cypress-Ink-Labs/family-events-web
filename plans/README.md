...
## Round 3 — fresh audit (2026-07-24, against `48f32f9`)

Generated by the improve skill (standard depth, 4 parallel auditors + vetting) after
plans 019-025 landed. Plans 026-046 are the vetted, selected batch — the three
maintenance bundles plus all six direction items. Execute in the order below; honor
each plan's STOP conditions and update its row. Every plan is `Issue: none (not
published)`.
...
- **029 depends on 028** (same file, `use-admin-events-realtime.ts`) — land 028 first.
...
- Two spec drifts were found and resolved at plan-writing time (2026-07-24), recorded in
  the affected plans: **028** — the canonical Sentry import is
  `import { Sentry } from "@/infrastructure/observability/sentry"` (not
  `import * as Sentry from "@sentry/react"`); **032** — `admin-comments.tsx` has four tabs
  (all/flagged/pending/approved), so `AdminCommentFilter` maps all four one-to-one.

### Round 3 — considered and rejected / deferred

- **Test-integrity bundle (presented to the maintainer, NOT selected this round)** —
  recorded so it is not re-audited blindly: copied-implementation tests in
  `profile.test.tsx` / `smart-image.test.ts` / `use-similar-events.test.ts`; e2e `/submit`
  route mismatch + runtime `test.skip()` false-greens + no CI e2e job; plan-for-today
  RPC-contract and admin-realtime lifecycle test gaps.
- **Map findings (deferred)** — map↔events bidirectional import, map interaction test
  gap, and map viewport→list rerender: MED confidence / no churn evidence, consistent
  with the standing map-view rejection ("not worth doing until there is").
...
| 023 | Migrate the community-event submission form to react-hook-form | P3 | M | 020 | DONE — merged `b0178be` ([PR #23](https://github.com/Cypress-Ink-Labs/family-events-web/pull/23)) · [CIL-101](https://linear.app/hexsleeves/issue/CIL-101) |
